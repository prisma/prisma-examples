...
There are are also the automated branches `dev` and `patch-dev`, which mirror the code from `master` (synced via a GitHub Action workflow), but uses the latest respective development channels of Prisma CLI and Prisma Client from npm instead (also updated via a GitHub Action workflow). Thanks to the test coverage of all projects, this can point us to incompatibilities early.
