# Prisma Postgres Example: Next.js 15 Starter (Webpack, Node.js + Vercel Edge, ESM)
...
- Next.js 15
  - Runtime:
    - Node.js 20.19.0 for normal server-side rendering (SSR) and API routes
...
- Prisma Client with two `prisma-client` generators
...
cd prisma-examples/prisma-postgres/nextjs-starter-webpack-nodejs-with-edge
npm install
```
...
# Prisma Postgres connection string (used for migrations and for Accelerate queries on the Vercel Edge runtime)
MIGRATE_DATABASE_URL="__YOUR_PRISMA_POSTGRES_CONNECTION_STRING__"

# Postgres connection string (used for queries by Prisma Client)
DIRECT_DATABASE_URL="__YOUR_PRISMA_POSTGRES_DIRECT_CONNECTION_STRING__"
