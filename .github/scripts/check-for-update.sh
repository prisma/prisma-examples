...
while [ $i -le $count ]; do
  ...
  if [ $code -eq 0 ]; then
    ...
    node .github/slack/notify.js "Prisma version $v released via the action https://github.com/prisma/prisma-examples/runs/$GITHUB_RUN_ID?check_suite_focus=true"
  fi
